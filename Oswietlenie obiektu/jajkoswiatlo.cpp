...
void instrukcja()
{
	...
	std::cout << "*** Sterowac polozeniem zrodla swiatla czerwonego mozna przy wcisnietym LPM, natomiast zrodla swiatla zielonego przy wcisnietym PPM ***" << std::endl;
	std::cout << "1 - wlaczenie tylko zrodla swiatla czerwonego" << std::endl;
	std::cout << "2 - wlaczenie tylko zrodla swiatla zielonego" << std::endl;
	std::cout << "3 - wlaczenie obu zrodel swiatla" << std::endl << std::endl;
...
void keys(unsigned char key, int x, int y)
{
	...
	if (key == '1')
	{
		mode = 1; glEnable(GL_LIGHT0); glDisable(GL_LIGHT1); lightmode = 1;
	}
	if (key == '2') 
	{
		mode = 1; glEnable(GL_LIGHT1); glDisable(GL_LIGHT0); lightmode = 2;
	}
	if (key == '3') 
	{		
		mode = 1;	glEnable(GL_LIGHT0); glEnable(GL_LIGHT1); lightmode = 3;
	}
	if (key == 'm') ustawienie();
	if (key == 'i') instrukcja();
	if (key == 'e') exit(0);
...
void main(void)
{
	std::cout << "*** i - Instrukcja ***" << std::endl << "p - punkty, w - siatka, s - wypelnione trojkaty,  c- czajnik" << std::endl << "1 - pierwsze swiatlo, 2 - drugie swiatlo, 3 - oba swiatla" << std::endl << "m - zmien czulosc myszy, e - wyjscie z programu" << std::endl;
	color();
